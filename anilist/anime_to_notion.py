...
query = """query {
   MediaListCollection(userName: "falconsensei", type: ANIME) {
    lists {
        name
      entries {
        id
...
def get_from_notion():
    notion_data = anotion.get_notion_data('ANIME')
    items = {}
...
def update_record(notion, an):
    ...
    if len(diff) > 0:
        print('Update:', an['Name'])
        notion_format = anotion.dict_to_notion(diff, anotion.anime_map_to_notion)
...
for an in animes_an:
    if an['Anilist Id'] in animes_notion:
        ...
        notion_format = anotion.dict_to_notion(an, anotion.anime_map_to_notion)
        anotion.insert_notion('ANIME', notion_format)